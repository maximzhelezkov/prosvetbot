...
from kb.kb import menuikb, menub5ikb
import logging
import asyncio
import sqlite3
...
async def callback_menub1(call: CallbackQuery):     
    ...
    logging.info(f"call = {call.data}, {call.from_user.id}, @{call.from_user.username or nonetype}")
    await bot.answer_callback_query(call.id)
    await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=START_MESSAGE)#, reply_markup=menuikb1)
...
async def callback_menub4(call: CallbackQuery):     
    ...
    logging.info(f"call = {call.data}, {call.from_user.id}, @{call.from_user.username or nonetype}")
    await bot.answer_callback_query(call.id)
    await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=MENUB1_MESSAGE)#, reply_markup=menuikb4)
